...
const PartyEnquiryTable = ({ data, loading, fetchData, showToast, viewItem, setViewItem }) => {
  ...
  // Helper to calculate weight dynamically in Edit Mode
  ...
  return (
    <>
      <style>{`
        ...
        .view-grid, .edit-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px; }
        .view-value-box {
          ...
          <div className="large-modal" onClick={e => e.stopPropagation()}>
            ...
            {isLoadingDetails ? (
                ...
                <form onSubmit={handleEditSave}>
                    ...
                    <div style={{display: 'flex', gap: '10px', marginBottom: '5px', fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase'}}>
                            ...
                            {viewItem.items.map((item, idx) => (
                                <tr key={idx}>
                                    ...
                                    <td style={{fontWeight: 600, color: '#475569'}}>
                                        {item.total_weight ? `${parseFloat(item.total_weight).toFixed(2)} kg` : '-'}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
