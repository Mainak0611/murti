...
const OrdersIndex = () => {
    ...
    dispatchForm.forEach((item, idx) => {
      ...
      if (type === 'supervisor') {
        ...
        if (idx === 0) {
          ...
          dmData.push(['TOT.\nWT(KG)', `${parseFloat(item.total_weight || 0).toFixed(2)}`]);
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      const rows = extItems.map(ext => {
        // First match against dispatchForm (items already in order)
        const orderIdx = matchExtractedItem(ext, dispatchForm, true);
        if (orderIdx !== -1) {
          const item = dispatchForm[orderIdx];
          return {
            ...
            isInOrder: true,
            addToOrder: false,
            orderedQtyForNewItem: ext.qty || ''
          };
        } else {
          // Match against all database items
          const dbIdx = matchExtractedItem(ext, availableItems, true);
          const matchedItem = dbIdx !== -1 ? availableItems[dbIdx] : null;
          return {
            tempId: Date.now() + Math.random(),
            extractedName: ext.item || '',
            extractedSize: ext.size || '',
            ...
            isInOrder: false,
            addToOrder: matchedItem ? true : false,
            orderedQtyForNewItem: ext.qty || ''
          };
        }
      });
  ...
  const handleExtractedRowChange = (index, field, value) => {
    ...
    if (field === 'matchedItemId') {
      const isInOrder = dispatchForm.some(item => String(item.item_id) === String(value));
      updated[index].isInOrder = isInOrder;
      if (isInOrder) {
        updated[index].addToOrder = false;
      } else {
        updated[index].addToOrder = value ? true : false;
      }
    }
  ...
  const handleAddExtractedRow = () => {
    setExtractedRows([
      ...extractedRows,
      {
        ...
        isInOrder: false,
        addToOrder: false,
        orderedQtyForNewItem: ''
      }
    ]);
  };

  const handleDeleteExtractedRow = (index) => {
    setExtractedRows(extractedRows.filter((_, idx) => idx !== index));
  };

  const handleApplyExtractedItems = async () => {
    setIsApplyingExtracted(true);
    try {
      // Find new items that the user chose to add to the order (addToOrder is true and isInOrder is false)
      const newItems = extractedRows.filter(row => row.addToOrder && !row.isInOrder && row.matchedItemId);

      let updatedOrder = selectedOrder;

      if (newItems.length > 0) {
        // Construct the list of items for the PUT request
        const existingItems = (selectedOrder.items || []).map(item => ({
          itemId: item.item_id,
          ordered_quantity: item.ordered_quantity
        }));

        const newlyAdded = newItems.map(row => ({
          itemId: parseInt(row.matchedItemId),
          ordered_quantity: parseInt(row.orderedQtyForNewItem) || parseInt(row.qty) || 0
        }));

        const updatedItemsPayload = [...existingItems, ...newlyAdded];

        // Call the PUT API to update the order's items
        await api.put(`/api/orders/${selectedOrder.id}`, {
          ...
          contact_no: selectedOrder.contact_no,
          remark: selectedOrder.remark,
          items: updatedItemsPayload
        });

        showToast("Order items updated with newly matched items!", "success");

        // Fetch the updated order from server
        ...
        fetchOrders(true); // Silent refresh orders index
      }

      // Update dispatchForm with updatedOrder items
      let latestDispatchForm = [];
      if (updatedOrder.items) {
        latestDispatchForm = updatedOrder.items.map(item => ({
          id: item.id,
          item_id: item.item_id,
          item_name: item.item_name,
          size: item.size,
      ...
      // Apply the reviewed quantities to current_dispatch
      const finalForm = latestDispatchForm.map(item => {
        // Find if this item has an extracted row
        const extRow = extractedRows.find(row => String(row.matchedItemId) === String(item.item_id));
        if (extRow) {
          return {
            ...item,
            current_dispatch: extRow.qty !== '' ? parseInt(extRow.qty) : ''
          };
        }
        return item;
      });

      setDispatchForm(finalForm);
      setExtractedRows([]);
      setShowReviewCard(false);
      showToast("Extracted quantities applied successfully!", "success");
    } catch (err) {
      console.error("Apply extracted items error:", err);
      showToast(err.response?.data?.error || "Failed to apply extracted items", "error");
    } finally {
  ...
  const handleSaveDispatch = async () => {
    ...
    if (!challanNo || challanNo.trim() === '') {
      ...
      showToast("Challan No. is required to submit a dispatch.", "error");
      return;
    }
      ...
      {selectedOrder && (
        <div className="modal-overlay" onClick={() => !isSaving && (setSelectedOrder(null), setSubmitAttempted(false))}>
          <div className="large-modal" onClick={e => e.stopPropagation()}>
            ...
            {/* --- Review Extracted Items Card --- */}
            {showReviewCard && (
              <div className="card fade-in-container" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  <div>
                    <h4 style={{ margin: 0, fontSize: '16px', fontWeight: '700', color: '#166534', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      📋 Review & Map Extracted Items
                    </h4>
                    <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: '#1b5e20' }}>
                      Review and map extracted text to database items. If the item is not already in the order, select "Add to order" to insert it.
                    </p>
                  </div>
                </div>

                <div style={{ overflowX: 'auto', background: '#ffffff', borderRadius: '6px', border: '1px solid #cbd5e1', marginBottom: '16px' }}>
                  <table className="items-table" style={{ margin: 0, width: '100%' }}>
                    <thead>
                      <tr>
                        <th style={{ width: '25%', padding: '10px 8px' }}>Extracted Text (Name & Size)</th>
                        <th style={{ width: '12%', textAlign: 'center', padding: '10px 8px' }}>Scan Qty</th>
                        <th style={{ width: '30%', padding: '10px 8px' }}>Mapped Database Item</th>
                        <th style={{ width: '10%', textAlign: 'center', padding: '10px 8px' }}>In Order?</th>
                        <th style={{ width: '18%', padding: '10px 8px' }}>Add to Order? / Qty</th>
                        <th style={{ width: '5%', textAlign: 'center', padding: '10px 8px' }}>Action</th>
                      </tr>
                    </thead>
                    <tbody>
                      {extractedRows.length === 0 ? (
                        <tr>
                          <td colSpan="6" style={{ textAlign: 'center', padding: '20px', color: '#64748b' }}>
                            No items extracted. You can manually add rows below or try scanning again.
                          </td>
                        </tr>
                      ) : (
                          ...
                          <tr key={row.tempId || idx}>
                            <td style={{ padding: '8px' }}>
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                <input
                                  ...
                                  onChange={(e) => handleExtractedRowChange(idx, 'extractedName', e.target.value)}
                                />
                                <input
                                  type="text"
                                  className="form-input"
                                  placeholder="Extracted Size"
                                  style={{ padding: '6px 8px', fontSize: '13px' }}
                                  value={row.extractedSize || ''}
                                  onChange={(e) => handleExtractedRowChange(idx, 'extractedSize', e.target.value)}
                                />
                              </div>
                            </td>
                            <td style={{ textAlign: 'center', padding: '8px' }}>
                              <input
                                type="text"
                                className="form-input"
                                style={{ width: '60px', padding: '6px', fontSize: '13px', textAlign: 'center', margin: '0 auto' }}
                                value={row.qty}
                            ...
                            <td style={{ padding: '8px' }}>
                              ...
                              >
                                <option value="">-- Mapped to Database Item --</option>
                            ...
                            <td style={{ padding: '8px' }}>
                              {row.isInOrder ? (
                                <span style={{ color: '#64748b', fontSize: '12px', fontWeight: '500' }}>Already in order</span>
                              ) : (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                  <label style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: '500' }}>
                                    <input
                                      type="checkbox"
                                      checked={!!row.addToOrder}
                                      onChange={(e) => handleExtractedRowChange(idx, 'addToOrder', e.target.checked)}
                                  ...
                                  {row.addToOrder && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                      <span style={{ fontSize: '11px', color: '#475569', fontWeight: '500' }}>Order Qty:</span>
                                      <input
                                        type="number"
                                        className="form-input"
                                        style={{ width: '70px', padding: '4px 6px', fontSize: '12px', textAlign: 'center' }}
                                        value={row.orderedQtyForNewItem || ''}
                                        onChange={(e) => handleExtractedRowChange(idx, 'orderedQtyForNewItem', e.target.value)}
                                        placeholder="Qty"
                                      />
                                    </div>
                                  )}
                            ...
                            <td style={{ textAlign: 'center', padding: '8px' }}>
                              <button
                                ...
                                style={{ padding: '6px 10px', color: '#ef4444', borderColor: '#fee2e2', background: '#fff' }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    ...
                    <button
                      ...
                      disabled={isApplyingExtracted}
                    >
                      {isApplyingExtracted ? '⏳ Applying...' : 'Apply Extracted Items'}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div style={{ overflowX: 'auto' }}>
              {(() => {
                ...
                const totalPrevSent = dispatchForm.reduce((sum, item) => {
                  const itemKey = item.item_name + '|' + (item.size || '');
                  const totalDispatched = dispatchHistory.reduce((innerSum, log) => {
                    const logKey = log.item_name + '|' + (log.size || '');
                    return logKey === itemKey ? innerSum + (log.quantity_sent || 0) : innerSum;
                  }, 0);
                  return sum + totalDispatched;
                }, 0);
                const totalPrevSentWeight = dispatchForm.reduce((sum, item) => {
                  const itemKey = item.item_name + '|' + (item.size || '');
                  const totalDispatched = dispatchHistory.reduce((innerSum, log) => {
                    const logKey = log.item_name + '|' + (log.size || '');
                    return logKey === itemKey ? innerSum + (log.quantity_sent || 0) : innerSum;
                  }, 0);
                  return sum + (totalDispatched * (parseFloat(item.unit_weight) || 0));
                }, 0);
                const totalBalance = dispatchForm.reduce((sum, item) => {
                  const itemKey = item.item_name + '|' + (item.size || '');
                  const totalDispatched = dispatchHistory.reduce((innerSum, log) => {
                    const logKey = log.item_name + '|' + (log.size || '');
                    return logKey === itemKey ? innerSum + (log.quantity_sent || 0) : innerSum;
                ...
                const totalAvailStock = dispatchForm.reduce((sum, item) => sum + (item.available_stock || 0), 0);
                const totalSendNow = dispatchForm.reduce((sum, item) => sum + (parseInt(item.current_dispatch) || 0), 0);
                const totalWeightSending = dispatchForm.reduce((sum, item) => {
                  const qty = parseInt(item.current_dispatch) || 0;
                  const sendingWeight = qty * (parseFloat(item.unit_weight) || 0);
                  return sum + sendingWeight;
                }, 0);
                  ...
                  <table className="items-table" style={{ width: '100%', minWidth: 900, borderCollapse: 'collapse', border: '1px solid #e2e8f0', borderRadius: '8px', overflow: 'hidden' }}>
                    <thead>
                      <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                        ...
                        <th style={{ padding: '14px 9px', textAlign: 'center', fontWeight: '700', color: '#475569', fontSize: '15px' }}>PREV. SENT WT</th>
                        ...
                        <th style={{ padding: '14px 9px', textAlign: 'center', fontWeight: '700', color: '#475569', fontSize: '15px' }}>AVAIL. STOCK</th>
                        <th style={{ padding: '14px 9px', textAlign: 'center', fontWeight: '700', color: '#475569', fontSize: '15px' }}>SEND NOW</th>
                        <th style={{ padding: '14px 9px', textAlign: 'center', fontWeight: '700', color: '#059669', fontSize: '15px' }}>WEIGHT (SENDING)</th>
                      </tr>
                    </thead>
                    <tbody>
                      {dispatchForm.map((item, idx) => {
                        ...
                        const prevSentWeight = totalDispatched * (parseFloat(item.unit_weight) || 0);
                        const balance = item.ordered_quantity - totalDispatched;
                        const balanceWeight = balance * (parseFloat(item.unit_weight) || 0);

                        let orderWeight = item.total_weight;
                        if (!orderWeight && item.unit_weight) {
                          orderWeight = item.unit_weight * item.ordered_quantity;
                        }
                        ...
                        const sendingWeight = (qty || 0) * (parseFloat(item.unit_weight) || 0);
                        ...
                        return (
                          <tr key={item.id || idx} style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: idx % 2 === 0 ? '#fff' : '#f9fafb' }}>
                            ...
                            <td style={{ padding: '14px 9px', textAlign: 'center', fontSize: '15px' }}>
                            ...
                            <td style={{ padding: '14px 9px', textAlign: 'center', backgroundColor: hasValue ? '#d1fae5' : 'transparent', transition: 'background-color 0.2s' }}>
                              <input
                                ...
                                placeholder="0"
                                style={{ width: '80px', textAlign: 'center', padding: '8px 5px', fontSize: '15px', margin: '0 auto' }}
                              />
                              {isStockExceeded && <span className="error-text" style={{ display: 'block', marginTop: '4px' }}>Exceeds Stock</span>}
                            </td>
                            <td style={{ padding: '14px 9px', textAlign: 'center', color: '#059669', fontWeight: '700', fontSize: '15px' }}>
                              {formatWeight(sendingWeight)}
                            </td>
                          </tr>
                        );
                      })}
                      ...
                      <tr style={{ borderTop: '2px solid #e2e8f0', backgroundColor: '#f8fafc' }}>
                        ...
                        <td style={{ padding: '14px 9px', textAlign: 'center', color: '#475569', fontWeight: '700', fontSize: '15px' }}>
                          {formatWeight(totalPrevSentWeight)}
                        </td>
                        <td style={{ padding: '14px 9px', textAlign: 'center', color: '#92400e', backgroundColor: '#fef3c7', fontWeight: '700', fontSize: '15px' }}>
                          {totalBalance}
                        </td>
                        ...
                        <td style={{ padding: '14px 9px', textAlign: 'center', fontSize: '15px' }}>
                          <span className={`stock-tag ${totalAvailStock === 0 ? 'stock-low' : ''}`}>{totalAvailStock}</span>
                        </td>
                        <td style={{ padding: '14px 9px', textAlign: 'center', color: '#334155', fontWeight: '700', fontSize: '15px' }}>
                          {totalSendNow}
                        </td>
                        <td style={{ padding: '14px 9px', textAlign: 'center', color: '#059669', backgroundColor: '#f0fdf4', fontWeight: '700', fontSize: '15px' }}>
                          {formatWeight(totalWeightSending)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
          ...
          <div style={{
            ...
            <p style={{ color: '#6b7280', marginBottom: 24, fontSize: 14 }}>
              {deleteTarget?.type === 'order'
                ...
                : 'Permanently delete this dispatch record?'}
            </p>
